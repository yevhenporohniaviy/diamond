      ...
      <div class="col-12 ">
        ...
        <div class="count-range mb-3">
          <div class="count-range">
            <div class="min-count">
              <input type="number" class="input-filter-number" pattern="[0-9]" v-model.number="ratio[0]" @change="selectNumberMobile(filter_item.alias, ratio)" :max="ratio[1]">
            </div>
            -
            <div class="max-count">
              <input type="number" class="input-filter-number" pattern="[0-9]" v-model.number="ratio[1]" @change="selectNumberMobile(filter_item.alias, ratio)" :max="ratio[1]">
            </div>
          </div>
        </div>
        <div class="my-4">
          <vue-slider
            ...
            v-bind="optionSlide"
            @drag-end="selectNumberMobile(filter_item.alias, ratio)">
          </vue-slider>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getValue(){
        this.price = [`${this.filter_item.values.min}`, `${this.filter_item.values.max}`]
      }
