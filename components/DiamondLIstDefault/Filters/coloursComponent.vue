      ...
      <div class="grid-content bg-purple w-100">
        <el-popover popper-class="popoverOther"
          ...
          <div class="container">
            ...
            <div class="col-12">
              <div class="w-100">
                ...
                <div class="count-range-string mt-5 mb-4" v-if="range.colours">
                  <div class="min-count-text">{{range.colours[0]}}</div>
                  -
                  <div class="max-count-text">
                    {{range.colours[range.colours.length-1]}}</div>
                </div>
                ...
                <vue-slider
                  ...
                  @drag-end="selectValue(filter_item.alias, colours)" @change="newValue(colours)" ></vue-slider>
          ...
          <el-button slot="reference" class="numberRange ">
            ...
            <div class="item-another-filter" v-if="range.colours">
              {{range.colours[0]}}
              -
              {{range.colours[range.colours.length-1]}}
            </div>
    ...
    methods: {
      ...
      inRange(obj){
        return this.filter_item.values
          .filter((val) => val.index >= this.colours[0] && val.index <= this.colours[1])
          .find((value) => value === obj)

      },
      getValue(){
        this.range.colours = [this.filter_item.values[0].value,this.filter_item.values[this.filter_item.values.length-1].value]
        this.colours = this.newIndex
      },
      setQueryValue(){
        if(typeof this.$store.state.diamonds.diamonds_query.colours !== 'undefined' && this.$store.state.diamonds.diamonds_query.colours.length > 0){
          let result = this.filter_item.values.filter((item) => {
            return this.$store.state.diamonds.diamonds_query.colours.includes(item.title);
          }).map((item) => item.index);
          this.range.colours = this.$store.state.diamonds.diamonds_query.colours
          if(result.includes(0)){
            this.colours = [ result[0], result[result.length - 1] + 0.5]
          }else{
            this.colours = [ result[0] - 0.5, result[result.length - 1] + 0.5]
          }
        }else{
          this.colours = this.newIndex
        }
