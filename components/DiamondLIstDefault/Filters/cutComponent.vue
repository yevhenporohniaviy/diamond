    ...
    <div class="grid-content bg-purple w-100">
      <el-popover popper-class="popoverOther"
        ...
        <div class="container">
            ...
            <div class="col-12">
              <div class="w-100">
                ...
                <div class="count-range-string mt-5 mb-4" v-if="range.cut">
                  <div class="min-count-text">{{range.cut[0]}}</div>
                  -
                  <div class="max-count-text">
                    {{range.cut[range.cut.length-1]}}</div>
                </div>
                ...
                <vue-slider
                  ...
                  @drag-end="selectValue(filter_item.alias, cut)" @change="newValue(cut)" ></vue-slider>
        ...
        <el-button slot="reference" class="numberRange ">
          ...
          <div class="item-another-filter" v-if="range.cut">
             {{range.cut[0]}}
            -
            {{range.cut[range.cut.length-1]}}
          </div>
    ...
    methods: {
      ...
      inRange(obj){
        return this.filter_item.values
          .filter((val) => val.index >= this.cut[0] && val.index <= this.cut[1])
          .find((value) => value === obj)
      },
      getValue(){
        this.range.cut = [this.filter_item.values[0].value, this.filter_item.values[this.filter_item.values.length-1].value]
        this.cut = this.newIndex
      },
      setQueryValue(){
        if(typeof this.$store.state.diamonds.diamonds_query.cut !== 'undefined' && this.$store.state.diamonds.diamonds_query.cut.length > 0){
          let result = this.filter_item.values.filter((item) => {
            return this.$store.state.diamonds.diamonds_query.cut.includes(item.title);
          }).map((item) => item.index);
          this.range.cut = this.$store.state.diamonds.diamonds_query.cut
          if(result.includes(0)){
            this.cut = [ result[0], result[result.length - 1] + 0.5]
          }else{
            this.cut = [ result[0] - 0.5, result[result.length - 1] + 0.5]
          }
        }else{
          if(this.$route.query.cut ){
            let result = this.filter_item.values.filter((item) => {
              return this.$route.query.cut.includes(item.title);
            }).map((item) => item.index);
            this.range.cut = this.$route.query.cut
            if(result.includes(0)){
              this.cut = [ result[0], result[result.length - 1] + 0.5]
            }else{
              this.cut = [ result[0] - 0.5, result[result.length - 1] + 0.5]
