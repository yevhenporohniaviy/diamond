          ...
          <div class="container">
            ...
            <div class="col-12">
              <div class="w-100">
                ...
                <div class="count-range-string mt-5 mb-4" v-if="range.polish">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getValue(){
        this.range.polish = [this.filter_item.values[0].value,this.filter_item.values[this.filter_item.values.length-1].value]
        this.polish = this.newIndex
      },
      setQueryValue(){
        if(typeof this.$store.state.diamonds.diamonds_query.polish !== 'undefined' && this.$store.state.diamonds.diamonds_query.polish.length > 0){
          let result = this.filter_item.values.filter((item) => {
            return this.$store.state.diamonds.diamonds_query.polish.includes(item.title);
          }).map((item) => item.index);
          this.range.polish = this.$store.state.diamonds.diamonds_query.polish
          if(result.includes(0)){
            this.polish = [ result[0], result[result.length - 1] + 0.5]
          }else{
            this.polish = [ result[0] - 0.5, result[result.length - 1] + 0.5]
          }
        }else{
          if(this.$route.query.polish ){
            let result = this.filter_item.values.filter((item) => {
              return this.$route.query.polish.includes(item.title);
            }).map((item) => item.index);
            this.range.polish = this.$route.query.polish
            if(result.includes(0)){
              this.polish = [ result[0], result[result.length - 1] + 0.5]
            }else{
              this.polish = [ result[0] - 0.5, result[result.length - 1] + 0.5]
